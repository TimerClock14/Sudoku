...
public class Cell {
	 ...
	 * @throws IllegalArgumentException if blk, col, or row do not contain cel
	 */
	public static void fillPencilMarks(Cell cel, Block blk, Column col, Row row){
		...
		// These will all throw NullPointerExceptions if cel == null
		if(!blk.contains(cel)){throw new IllegalArgumentException("blk does not contain cel");}
		if(!col.contains(cel)){throw new IllegalArgumentException("col does not contain cel");}
		if(!row.contains(cel)){throw new IllegalArgumentException("row does not contain cel");}
